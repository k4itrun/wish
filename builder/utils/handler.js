...
class Builder {
    ...
    async CreateBuild(dirPath) {
        ...
        try {
            ...
            const filesToRemove = [
                path.join(exeDist, 'builder-debug.yml'),
                path.join(exeDist, 'builder-effective-config.yaml'),
                path.join(exeDist, `win-unpacked`)
            ];
    ...
    async createAsk(qst) {
        try {
            ...
            const answer = await new Promise((resolve) => {
                const questionText = utils.applyGradient(
                    ['#fcca7e', '#ed7efc', '#7eb0fc', '#7ee0fc'],
                    `Question ${this.currentQst}: ${qst}`
                );
    ...
    async start() {
        try {
            ...
            setTimeout(async () => {
                ...
                console.clear();
                console.log(utils.applyGradient(
                    ['#FFFFFF', '#E0BBE4', '#957DAD', '#D291BC', '#F17EF7', '#8A2BE2', '#af45fa'],
                    utils.WishBanner()
                ));
