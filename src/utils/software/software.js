...
const GetDisksInfo = async () => {
  try {
    ...
    const header = ['Drive'.padEnd(column), 'Free'.padEnd(column), 'Total'.padEnd(column), 'Use'.padEnd(column)].join('');
...
const GetLocalIP = async () => {
  try {
    const output = await program.ExecPowerShell('(Resolve-DnsName -Name myip.opendns.com -Server 208.67.222.220).IPAddress');
    return output;
  } catch (error) {
    console.error(error);
    return 'Not Found';
