  ...
  for (const [gameName, gamePaths] of Object.entries(gamesPaths.GetGames())) {
   ...
   for (const [name, relativePath] of Object.entries(gamePaths)) {
    ...
    try {
     ...
     const gameDestPath = isDirectory ? path.join(destGames, name) : path.join(destGames, name, path.basename(gamePath));
 ...
 if (fs.existsSync(config)) {
  ...
  for (const accountId of accountIds) {
   try {
    ...
    const accountResponse = await axios.get(`https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=${KEY_STEAM}&steamids=${accountId}`);
    const gamesResponse = await axios.get(`https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key=${KEY_STEAM}&steamids=${accountId}`);
    const levelResponse = await axios.get(`https://api.steampowered.com/IPlayerService/GetSteamLevel/v1/?key=${KEY_STEAM}&steamids=${accountId}`);

    const accountInfo = accountResponse.data?.response || {};
    const { players = [] } = accountInfo;

    const accountGames = gamesResponse.data?.response || {};
    const { game_count = "Not Found" } = accountGames;

    const accountLevel = levelResponse.data?.response || {};
    const { player_level = "Not Found" } = accountLevel;
